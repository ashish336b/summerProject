  ...
  <adminSidebar>
    <div>
      <div class="columns is-multiline">
        ...
        <div class="column is-1">
          <nuxt-link to="/admin/invoice/add" class="button is-small is-primary"
            >Add</nuxt-link
          >
        </div>
      </div>
    ...
    >
      <template slot-scope="item">
        <a class="button is-small is-primary" @click="goToInvoice(item)">
          Details
        </a>
        <a
          v-if="item.isCredit"
    ...
    mark: function (item) {
      if (item.paidDate) {
        Swal.fire({
          ...
          if (result.value) {
            this.$axios
              .put(`/crm/invoice/unmarkAsPaid/${item._id}`, {
                date: "1998/2/13",
              })
              .then(({ data }) => {
                if (!data.error)
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            this.$axios
              .put(`/crm/invoice/markAsPaid/${item._id}`, {
                date: this.date,
              })
              .then(({ data }) => {
                console.log(data);
